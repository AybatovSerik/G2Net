from G2Net.G2Net.custom_blocks import BigConvBlock4Input, Dropout4Input, BigConvBlock
...
def ConvModelFor3Input512(big_conv_num=3,
                       filters_sequence=[[4,8],[8,16],[16,32]],
                       kernel_size_sequence=[3,3]*3,
                       pooling_factors=[4, 2, 2],
                       activations=['selu']*3,
    ...
    assert len(conv_nums)==big_conv_num, f"conv_nums for {len(conv_nums)} blocks, but big_conv_num is {big_conv_num}"
    assert len(dropouts)==big_conv_num, f"dropouts for {len(dropouts)} blocks, but big_conv_num is {big_conv_num}"
    input_0 = Input(shape=[512, 15])
    input_1 = Input(shape=[512, 15])
    input_2 = Input(shape=[512, 15])
    x_0 = input_0
    x_1 = input_1
    x_2 = input_2
    x_all = Concatenate()([x_0, x_1, x_2])
...
def ConvModelFor3InputOneFlow(big_conv_num=5,
                       filters_sequence=[[4,8],[8,16],[16,32],[32,64],[64,128]],
                       kernel_size_sequence=[[3,3],[3,3],[3,3],[3,3],[3,3]],
                       pooling_factors=[2,2,2,2,2],
                       activations=['relu','relu','relu','relu','relu'],
    ...
    input_1 = Input(shape=[4096, 1])
    input_2 = Input(shape=[4096, 1])
    x_all = Concatenate()([input_0, input_1, input_2])
    for block_num in range(big_conv_num):
        x_all = BigConvBlock(
